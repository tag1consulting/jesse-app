...
enum AppModelContainer {
    ...
    static let shared: ModelContainer = {
        if let container = try? ModelContainer(for: JesseThread.self, Turn.self, TurnAttachment.self, WrittenMeal.self) {
            return container
        }
        ...
        if let memory = try? ModelContainer(
            for: JesseThread.self, Turn.self, TurnAttachment.self, WrittenMeal.self,

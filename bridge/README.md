...
| `vaultnotes` (namespaced) | `GET /jesse/vault/note`: one note by `path` or wiki `target`, with the sha256 of the whole file as its ETag, so the app opens the Studio's copy whenever the phone's Obsidian folder is behind. Owns the path rules (`.md` under the notes root, no dot segments, no `_to-purge/`, canonical path confined). See SECURITY.md |
| `vaultwrites` (namespaced) | `POST /jesse/vault/writes`: every write the app makes to a note (edit, tick, untick, capture), applied in order against the current file: three way merge by line (`diffy`), ticks found by content, captures deduplicated, conflicts answered with the current text. Applied ids in `<state_dir>/vault-writes.json` for 30 days. Never `Today.md`, never git |
...
## The Strands board (`GET /jesse/strands`, the nightly audit)

The vault keeps one status note per piece of work under `vault/Strands/`
(`vault/Strands/archive/` is ignored). The bridge parses them, read only, and
...
that changes nothing costs a `304`. A missing `Strands/` is `200` with empty lists.

**The nightly file.** `vault/Inbox/YYYY-MM-DD-strands-audit.md`, written at
...
for the day, never touches a Strands note, and never calls the network. No
`Strands/` directory: nothing is written and one warning is logged.
...
rewrite the status line, ask one question if it has to. The bridge never closes a step or
edits a draft itself; the only strand note edit it applies is the app's own tick, written
through `POST /jesse/vault/writes` exactly as the phone made it. Ticks arrive three ways and
share one ledger, keyed on (note, step id):
...
| The app's write outbox | `POST /jesse/vault/writes` with a `tick` or `untick` record for `Strands/<Note>.md`. Once the record is applied, `strandticks::report_app_tick` reports the step, the same call the route below makes. |
| The app, to an older bridge | `POST /jesse/strands/:slug/ticks` with `{ "id": "P1", "checked": true }`, sent by the outbox only when the bridge has no write route. `202` with `{ "state" }`: `pending`, `cancelled` (an untick took back a pending tick), `already_fired`, `nothing`, or `done` (the step is already in Done here). `404` for an unknown note or step, or a slug carrying a path. |

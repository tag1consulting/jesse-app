...
/// diet day lands in Apple Health (`HealthAutoTurn.morningRefresh`). The automatic thread
/// carries `ThreadOrigin.automatic` instead of a prefix, so the text stays byte-identical,
/// and both write `lastFiredDayKey`.
public enum HealthNewDay {
    ...
    /// Only the automatic weigh-in is stopped by it; the button reads it to say a run would
    /// be a second one, and sends anyway. A per-device date, not a secret. iOS only: the
    /// Mac's button keeps its old behaviour.

...
# --- LOCAL PATCH, not in the committed script -------------------------------
# Every xcodebuild runs under a PTY (`script`) so its output is LINE buffered: through a
# plain file redirect it block-buffers, and minutes of buffered silence is indistinguishable
# from the post-summary hang this guard is here to kill. STALL_SECS of true silence kills it.
XCB_REAL="$(command -v xcodebuild)"
STALL_SECS="${STALL_SECS:-420}"
xcodebuild() {
  local log rc pid watcher
  log="$(mktemp -t xcblog)"
  script -q /dev/null "$XCB_REAL" "$@" >"$log" 2>&1 &
  pid=$!
  (
    last=0; same=0
    while kill -0 "$pid" 2>/dev/null; do
      sleep 15
      sz=$(wc -c <"$log" | tr -d ' ')
      if [ "$sz" = "$last" ]; then same=$((same+15)); else same=0; last=$sz; fi
      if [ "$same" -ge "$STALL_SECS" ]; then
        echo "STALL-GUARD: no output for ${STALL_SECS}s, killing xcodebuild" >>"$log"
  ...
  echo "XCBLOG: $log (rc=$rc)"
  grep -E "^Test Suite '[A-Za-z ]+\\.xctest' (passed|failed)|Executed [0-9]+ tests, with|^\\*\\* (BUILD|TEST)|^Test Case .* failed|STALL-GUARD" "$log" | tail -40
  return $rc
}
...
run_step() {
  ...
  if "$@"; then
    ...
    echo "local-ci-macos: step FAILED: '${name}' (continuing, NOFAIL mode)" >&2
  fi
}
...
summary
echo ""
echo "local-ci-macos: NOFAIL mode finished — read the summary above."
